...
def to_union_call(instring, tokensStart, retTokens):
    ...
    if len(unions) == 1:
        ...
        if any(o.lower().replace(" ", "_") not in ['union_all', 'union', 'intersect', 'except'] for o in operators):
            raise Exception("Unrecognized query union operator: {}".format(operators))

        assert(len(sources) > 0)
        parse, last_op = sources[:1], None
        for i, (_op, _source) in enumerate(zip(operators, sources[1:])):
            op = _op.lower().replace(" ", "_")
            if last_op is None or op == last_op:
                parse.append(_source)
            else:
                parse = [{last_op: parse}, _source]
            last_op = op

        if not tok.get('orderby') and not tok.get('limit'):
            return {op: parse}
        else:
            output = {"from": {op: parse}}
...
unordered_sql = Group(
    FROM.suppress().setDebugActions(*debug) + (
            delimitedList(Group(table_source)) + ZeroOrMore(join))("from") +
    Optional(WHERE.suppress().setDebugActions(*debug) + expr.setName("where"))("where") +
    Optional(GROUP_BY.suppress().setDebugActions(*debug) + delimitedList(Group(selectColumn))(
    ...
    Optional(ORDER_BY.suppress().setDebugActions(*debug) + delimitedList(Group(sortColumn))(
...
ordered_sql << Group(
    Group(Group(
        unordered_sql +
        ZeroOrMore((UNION_ALL | UNION | EXCEPT | INTERSECT) + unordered_sql)
    )("union"))("from") +
